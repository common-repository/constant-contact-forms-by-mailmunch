...
class Constantcontact_Mailmunch_Rewrite {
  ...
  public function on_save_post($postId) {
    if (get_post_type($postId) == CONSTANTCONTACT_MAILMUNCH_POST_TYPE) {
  ...
  public function on_post_type_link($permalink) {
    ...
    foreach ($customPostTypes as $type => $postType) {
      if ($type == CONSTANTCONTACT_MAILMUNCH_POST_TYPE) {
  ...
  public function enable_front_page_landing_pages($query) {
    ...
    if ('' == $postType && 0 != $query->query_vars['page_id']) {
      $query->query_vars['post_type'] = array('page', CONSTANTCONTACT_MAILMUNCH_POST_TYPE);
    }
  }
